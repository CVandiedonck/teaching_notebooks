...
# pour aller encore plus loin, je comprends l'usage de la fonction paste0 (documentée dans la même aide que pour la fonction `paste()`) (même fonction avec sep="" par défaut)
...
# Pour aller plus loin, sI vous n'aviez pas fait la conversion numérique, la fonction sum() aurait retourné une erreur. Vérifions-le en reconverstissant en chaîne de caractères:
results_bias <- as.character(results_bias)
print(results_bias)
sum(results_bias)
#Avant de faire des opérations dans R, vérifiez donc bien que vos vecteurs sont numériques (ou entiers!) avec les fonction mode, ìs.numeric ou ìs.integer!
mode(results_bias)
...
precip[les4villes] # retourne les precipitations dans l'ordre choisi des villes = identique a la ligne 200 de ce script!
# notez bien la différence dans l'odre d'affichage: lorsque vous utilisez le vecteur les4villes, l'ordre souhaité est respecté; lorsque vous recherchez par indice, l'ordre affiché est celui de l'ordre croissant des indices des villes dans le vecteur precip
